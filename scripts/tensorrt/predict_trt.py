...
class Inference_TensorRT:
    def __init__(self, onnx_file_path,
        ...
        for binding in self.engine:
            if self.engine.binding_is_input(binding):
                self.input_shapes.append(tuple(list(self.engine.get_binding_shape(binding))))
            else:
                self.output_shapes.append(tuple(list(self.engine.get_binding_shape(binding))))
    ...
    def do_inference(self, image, score_threshold=0.4, top_k=10000, NMS_threshold=0.4, NMS_flag=True, skip_scale_branch_list=[]):
        ...
        allocate_place = numpy.prod(input_batch.shape)
        self.inputs[0].host[:allocate_place] = input_batch.flatten(order='C').astype(np.float32)

        [cuda.memcpy_htod_async(inp.device, inp.host, self.stream) for inp in self.inputs]
        self.executor.execute_async(batch_size=self.engine.max_batch_size, bindings=self.bindings,stream_handle=self.stream.handle)
...
def run_prediction_folder():
    ...
    im = cv2.imread('debug_image/3.jpg')
